...
public class AddMusic extends JFrame implements ActionListener{
	...
	//定义所需要的组件
	JPanel jp_add,jp1;
	JLabel jlb_title,jlb_songname,jlb_songaddress,jlb_singer,jlb_songalbum,jlb_songlrc;
	public JTextField jtf_songname,jtf_songaddress, jtf_singername, jtf_songlrc,jtf_songalbum;
	...
	public AddMusic()
	{
		...
		jlb_songalbum = new JLabel("专  辑:",JLabel.CENTER);
		...
		jlb_songalbum.setBounds(100, 220, 60, 25);
		...
		jtf_songalbum=new JTextField(40);
		jtf_songalbum.setText("专辑名~");
		jtf_songalbum.setForeground(Color.darkGray);
		jtf_songalbum.getFocusTraversalKeysEnabled();
		...
		jtf_songalbum.setBounds( 200, 220, 150, 28);
		...
		jp_add.add(jlb_songalbum);
		jp_add.add(jtf_songalbum);
		...
		//设置背景图片
		JPanel imagePanel;
		ImageIcon background=new ImageIcon("icon/333.jpg");
...
//		}
		...
		if(e.getSource()==jb_sure)
		{
			...
			String songlist=this.getJtf_songalbum().getText();
			...
			if (songname == null || " ".equals(songname)) {
				...
				String musicName = jtf_songname.getText() ;
				String songAddress = jtf_songaddress.getText();
				String singerName = jtf_singername.getText();
				String songAlbum = jtf_songalbum.getText();
				String songLrc = jtf_songlrc.getText();
				if(mm.insert(musicName,songAddress,singerName,songAlbum,songLrc)) {
					this.dispose();
					//new HubFrame();
					//SongListPanel.add(musicName);
				}
	...
	public JTextField getJtf_songalbum() {
		return jtf_songalbum;
	}
	public void setJtf_songalbum(JTextField jtf_songalbum) {
		this.jtf_songalbum = jtf_songalbum;
	}
